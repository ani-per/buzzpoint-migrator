...
const insertPlayerStatement = db.prepare('INSERT INTO player (team_id, name, slug, question_set_id) VALUES (?, ?, ?, ?)');
...
const findQuestionSetByIdStatement = db.prepare('SELECT * FROM question_set WHERE id = ?');
const findQuestionSetByNameStatement = db.prepare('SELECT * FROM question_set WHERE name = ?');
...
const findPacketByNumberStatement = db.prepare('SELECT id FROM packet WHERE question_set_edition_id = ? and number = ?');
        ...
        for (const subFolder of subFolders) {
            ...
            try {
                ...
                try {
                    ...
                    const gameFileDirents = filterFiles(await fs.readdir(gamesFilePath, { withFileTypes: true, recursive: true }), "qbj");
                            ...
                            try {
                                ...
                                for (let { team } of gameData.match_teams) {
                                    ...
                                    if (!teamDictionary[teamName]) {
                                        const { lastInsertRowid: teamId } = insertTeamStatement.run(tournamentId, teamName, slugify(teamName, slugifyOptions));
                                ...
                                console.warn(`\tCouldn't find packet ${packetDescriptor} in edition ${edition} for game ${gameFile.replaceAll(".qbj", "")}.`);
                                console.log(err);
                            }
                        } catch (err) {
